...
class PnioProtocolDevice(Protocol):
    ...
    @classmethod
    def _start_server(cls, address, tags, device, db_path, device_name):
        """Start a cpppo enip server.
        ...
        try:
            cmd = PnioProtocolDevice._start_server_cmd(device, db_path, device_name)
            print(" ".join(cmd))
            server = subprocess.Popen(" ".join(cmd), shell=True)
    ...
    # TODO: parametric PRINT_STDOUT and others
    @classmethod
    def _start_server_cmd(cls, device, db_path, device_name):

        CMD = 'cd profinet_device/build/; sudo ./pn_dev '
        # print 'DEBUG: enip _start_server_cmd HTTP: ', HTTP
        DBNAME = '-y ' + PN_NAME + ' '
        DBPATH = '-x ../../' + db_path + ' '
        PID = '-z ' + '1'
