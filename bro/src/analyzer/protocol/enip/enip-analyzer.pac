...
refine flow ENIP_Flow += {
	...
	# EXCEPTION
	function deliver_Exception(header: ENIP_Header, message: Exception): bool
		%{
		if ( ::enip_exception )
			{
			BifEvent::generate_enip_exception(connection()->bro_analyzer(),
			                                    connection()->bro_analyzer()->Conn(),
			                                    HeaderToBro(header),
			                                    ${message.code});
			}
	...
	# REQUEST CMD=0
	function deliver_NopRequest(header: ENIP_Header, message: NopRequest): bool
		%{
		if ( ::enip_nop_request )
			{
			BifEvent::generate_enip_nop_request(connection()->bro_analyzer(),
							connection()->bro_analyzer()->Conn(),
							HeaderToBro(header),
